...
function doSearch( queryStruct, first, total ) {
    ...
    if ( isCacheValid(queryStruct) && isPresentInCache(first, total) ) {
        //Get all data from cache
        setResultData( getCachedData(first, total), first );
    }
    else {
...
function onPageChangedHandle(e, oldPage, newPage) {
    ...
    doSearch( {textQuery: cacheStruct.currentText, structQuery: cacheStruct.currentStructure}, rangeFrom, numResultsToShow );
}
...
window.onload = function() {
    ...
    sketcher = new ChemDoodle.SketcherCanvas('sketcher', 500, 300, {oneMolecule:false});
